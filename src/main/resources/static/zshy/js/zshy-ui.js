...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.QYSK_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
    	...
    	common: {
            ...
            // 数据字典转下拉框
            dictToSelect: function(datas, value, name,required) {
            	var actions = [];
            	actions.push($.common.sprintf("<select class='form-control' name='%s' '%s'>", name,required));
                $.each(datas, function(index, dict) {
